...
var readFileSync = sinon.stub().returns(fs.readFileSync(path.resolve(__dirname, '../data/axe.conf')));
var AxeService = proxyquire('../../lib/services/axed', {
  fs: {
    readFileSync: readFileSync
  }
});
var defaultAxeConf = fs.readFileSync(path.resolve(__dirname, '../data/default.axe.conf'), 'utf8');

describe('Axe Service', function() {
  ...
  describe('@constructor', function() {
    ...
    it('will create an instance without `new`', function() {
      var axed = AxeService(baseConfig);
      should.exist(axed);
    });
    it('will init caches', function() {
      var axed = new AxeService(baseConfig);
      should.exist(axed.utxosCache);
      should.exist(axed.txidsCache);
      should.exist(axed.balanceCache);
      should.exist(axed.summaryCache);
      should.exist(axed.transactionDetailedCache);
      should.exist(axed.masternodeListCache);

      should.exist(axed.transactionCache);
      should.exist(axed.rawTransactionCache);
      should.exist(axed.blockCache);
      should.exist(axed.rawBlockCache);
      should.exist(axed.blockHeaderCache);
      should.exist(axed.zmqKnownTransactions);
      should.exist(axed.zmqKnownBlocks);
      should.exist(axed.lastTip);
      should.exist(axed.lastTipTimeout);
    });
    it('will init clients', function() {
      var axed = new AxeService(baseConfig);
      axed.nodes.should.deep.equal([]);
      axed.nodesIndex.should.equal(0);
      axed.nodes.push({client: sinon.stub()});
      should.exist(axed.client);
    });
    it('will set subscriptions', function() {
      var axed = new AxeService(baseConfig);
      axed.subscriptions.should.deep.equal({
        address: {},
        rawtransaction: [],
        hashblock: [],
        transactionlock: []
  ...
  describe('#_initDefaults', function() {
    it('will set transaction concurrency', function() {
      var axed = new AxeService(baseConfig);
      axed._initDefaults({transactionConcurrency: 10});
      axed.transactionConcurrency.should.equal(10);
      axed._initDefaults({});
      axed.transactionConcurrency.should.equal(5);
    });
  });

  describe('@dependencies', function() {
    it('will have no dependencies', function() {
      AxeService.dependencies.should.deep.equal([]);
    });
  });
  ...
  describe('#getPublishEvents', function() {
    it('will return spec', function() {
      var axed = new AxeService(baseConfig);
      var events = axed.getPublishEvents();
      should.exist(events);
      events.length.should.equal(4);
      events[0].name.should.equal('axed/rawtransaction');
      events[0].scope.should.equal(axed);
      events[0].subscribe.should.be.a('function');
      events[0].unsubscribe.should.be.a('function');
      events[1].name.should.equal('axed/transactionlock');
      events[1].scope.should.equal(axed);
      events[1].subscribe.should.be.a('function');
      events[1].unsubscribe.should.be.a('function');
      events[2].name.should.equal('axed/hashblock');
      events[2].scope.should.equal(axed);
      events[2].subscribe.should.be.a('function');
      events[2].unsubscribe.should.be.a('function');
      events[3].name.should.equal('axed/addresstxid');
      events[3].scope.should.equal(axed);
    ...
    it('will call subscribe/unsubscribe with correct args', function() {
      var axed = new AxeService(baseConfig);
      axed.subscribe = sinon.stub();
      axed.unsubscribe = sinon.stub();
      var events = axed.getPublishEvents();

      events[0].subscribe('test');
      axed.subscribe.args[0][0].should.equal('rawtransaction');
      axed.subscribe.args[0][1].should.equal('test');

      events[0].unsubscribe('test');
      axed.unsubscribe.args[0][0].should.equal('rawtransaction');
      axed.unsubscribe.args[0][1].should.equal('test');

      events[1].subscribe('test');
      axed.subscribe.args[1][0].should.equal('transactionlock');
      axed.subscribe.args[1][1].should.equal('test');

      events[1].unsubscribe('test');
      axed.unsubscribe.args[1][0].should.equal('transactionlock');
      axed.unsubscribe.args[1][1].should.equal('test');

      events[2].subscribe('test');
      axed.subscribe.args[2][0].should.equal('hashblock');
      axed.subscribe.args[2][1].should.equal('test');

      events[2].unsubscribe('test');
      axed.unsubscribe.args[2][0].should.equal('hashblock');
      axed.unsubscribe.args[2][1].should.equal('test');
    });
  });
  ...
  describe('#subscribe', function() {
    ...
    it('will push to subscriptions', function() {
      var axed = new AxeService(baseConfig);
      var emitter = {};
      axed.subscribe('hashblock', emitter);
      axed.subscriptions.hashblock[0].should.equal(emitter);

      var emitter2 = {};
      axed.subscribe('rawtransaction', emitter2);
      axed.subscriptions.rawtransaction[0].should.equal(emitter2);
    });
  });
  ...
  describe('#unsubscribe', function() {
    ...
    it('will remove item from subscriptions', function() {
      ...
      var emitter4 = {};
      var emitter5 = {};
      axed.subscribe('hashblock', emitter1);
      axed.subscribe('hashblock', emitter2);
      axed.subscribe('hashblock', emitter3);
      axed.subscribe('hashblock', emitter4);
      axed.subscribe('hashblock', emitter5);
      axed.subscriptions.hashblock.length.should.equal(5);

      axed.unsubscribe('hashblock', emitter3);
      axed.subscriptions.hashblock.length.should.equal(4);
      axed.subscriptions.hashblock[0].should.equal(emitter1);
      axed.subscriptions.hashblock[1].should.equal(emitter2);
      axed.subscriptions.hashblock[2].should.equal(emitter4);
      axed.subscriptions.hashblock[3].should.equal(emitter5);
    });
    it('will not remove item an already unsubscribed item', function() {
      var axed = new AxeService(baseConfig);
      var emitter1 = {};
      var emitter3 = {};
      axed.subscriptions.hashblock= [emitter1];
      axed.unsubscribe('hashblock', emitter3);
      axed.subscriptions.hashblock.length.should.equal(1);
      axed.subscriptions.hashblock[0].should.equal(emitter1);
    });
  });
  ...
  describe('#subscribeAddress', function() {
    ...
    it('will not an invalid address', function() {
      ...
      axed.subscribeAddress(emitter, ['invalidaddress']);
      should.not.exist(axed.subscriptions.address['invalidaddress']);
    });
    it('will add a valid address', function() {
      var axed = new AxeService(baseConfig);
      var emitter = new EventEmitter();
      axed.subscribeAddress(emitter, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      should.exist(axed.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
    });
    it('will handle multiple address subscribers', function() {
      var axed = new AxeService(baseConfig);
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      axed.subscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      axed.subscribeAddress(emitter2, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      should.exist(axed.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      axed.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(2);
    });
    it('will not add the same emitter twice', function() {
      ...
      axed.subscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      should.exist(axed.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      axed.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
    });
  });
  ...
  describe('#unsubscribeAddress', function() {
    ...
    it('it will remove a subscription', function() {
      ...
      should.exist(axed.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      axed.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(2);
      axed.unsubscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      axed.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
    });
    it('will unsubscribe subscriptions for an emitter', function() {
      ...
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      axed.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      axed.unsubscribeAddress(emitter1);
      axed.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
    });
    it('will NOT unsubscribe subscription with missing address', function() {
      ...
      var emitter2 = new EventEmitter();
      axed.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      axed.unsubscribeAddress(emitter1, ['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs']);
      axed.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(2);
    });
    it('will NOT unsubscribe subscription with missing emitter', function() {
      ...
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      axed.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter2];
      axed.unsubscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      axed.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
      axed.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'][0].should.equal(emitter2);
    });
    it('will remove empty addresses', function() {
      ...
      axed.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      axed.unsubscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      axed.unsubscribeAddress(emitter2, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      should.not.exist(axed.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
    });
    it('will unsubscribe emitter for all addresses', function() {
      ...
      var emitter2 = new EventEmitter();
      axed.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      axed.subscriptions.address['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs'] = [emitter1, emitter2];
      sinon.spy(axed, 'unsubscribeAddressAll');
      axed.unsubscribeAddress(emitter1);
      axed.unsubscribeAddressAll.callCount.should.equal(1);
      axed.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
      axed.subscriptions.address['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs'].length.should.equal(1);
    });
  });
  ...
  describe('#unsubscribeAddressAll', function() {
    ...
    it('will unsubscribe emitter for all addresses', function() {
      ...
      axed.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      axed.subscriptions.address['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs'] = [emitter1, emitter2];
      axed.subscriptions.address['mgY65WSfEmsyYaYPQaXhmXMeBhwp4EcsQW'] = [emitter2];
      axed.subscriptions.address['7d5169eBcGHF4BYC6DTffTyeCpWbrZnNgz'] = [emitter1];
      axed.unsubscribeAddress(emitter1);
      axed.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
      axed.subscriptions.address['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs'].length.should.equal(1);
      axed.subscriptions.address['mgY65WSfEmsyYaYPQaXhmXMeBhwp4EcsQW'].length.should.equal(1);
      should.not.exist(axed.subscriptions.address['7d5169eBcGHF4BYC6DTffTyeCpWbrZnNgz']);
    });
  });
  ...
  describe('#_getDefaultConfig', function() {
    it('will generate config file from defaults', function() {
      var axed = new AxeService(baseConfig);
      var config = axed._getDefaultConfig();
      config.should.equal(defaultAxeConf);
    });
  });
  ...
  describe('#_loadSpawnConfiguration', function() {
    ...
    it('will parse a axe.conf file', function() {
      var TestAxe = proxyquire('../../lib/services/axed', {
        fs: {
          readFileSync: readFileSync,
      ...
      var axed = new TestAxe(baseConfig);
      axed.options.spawn.datadir = '/tmp/.axe';
      var node = {};
      axed._loadSpawnConfiguration(node);
      should.exist(axed.spawn.config);
      axed.spawn.config.should.deep.equal({
        addressindex: 1,
        checkblocks: 144,
        dbcache: 8192,
    ...
    it('will expand relative datadir to absolute path', function() {
      ...
      var config = {
        node: {
          network: axecore.Networks.testnet,
          configPath: '/tmp/.axecore/axecore-node.json'
        },
        spawn: {
          datadir: './data',
          exec: 'testpath'
      ...
      var axed = new TestAxe(config);
      axed.options.spawn.datadir = './data';
      var node = {};
      axed._loadSpawnConfiguration(node);
      axed.options.spawn.datadir.should.equal('/tmp/.axecore/data');
    });
    it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
      var TestAxe = proxyquire('../../lib/services/axed', {
        fs: {
          readFileSync: sinon.stub().returns(fs.readFileSync(__dirname + '/../data/badaxe.conf')),
          existsSync: sinon.stub().returns(true),
        },
        mkdirp: {
          sync: sinon.stub()
      ...
      var axed = new TestAxe(baseConfig);
      (function() {
        axed._loadSpawnConfiguration({datadir: './test'});
      }).should.throw(axecore.errors.InvalidState);
    });
  ...
  describe('#_checkConfigIndexes', function() {
    ...
    it('should warn the user if reindex is set to 1 in the axe.conf file', function() {
      var axed = new AxeService(baseConfig);
      var config = {
        txindex: 1,
        addressindex: 1,
      ...
      var node = {};
      axed._checkConfigIndexes(config, node);
  ...
  describe('#_resetCaches', function() {
    it('will reset LRU caches', function() {
      ...
      for (var i = 0; i < 10; i++) {
        ...
        axed.transactionDetailedCache.set(keys[i], {});
        axed.utxosCache.set(keys[i], {});
        axed.txidsCache.set(keys[i], {});
        axed.balanceCache.set(keys[i], {});
        axed.summaryCache.set(keys[i], {});
      }
      axed._resetCaches();
      should.equal(axed.transactionDetailedCache.get(keys[0]), undefined);
      should.equal(axed.utxosCache.get(keys[0]), undefined);
      should.equal(axed.txidsCache.get(keys[0]), undefined);
      should.equal(axed.balanceCache.get(keys[0]), undefined);
      should.equal(axed.summaryCache.get(keys[0]), undefined);
    });
  });
  ...
  describe('#_tryAllClients', function() {
    it('will retry for each node client', function(done) {
      ...
      axed._tryAllClients(function(client, next) {
        client.getInfo(next);
      }, function(err) {
        if (err) {
          return done(err);
        }
        axed.nodes[0].client.getInfo.callCount.should.equal(1);
        axed.nodes[1].client.getInfo.callCount.should.equal(1);
        axed.nodes[2].client.getInfo.callCount.should.equal(1);
        done();
      });
    });
  ...
  describe('#_wrapRPCError', function() {
    it('will convert axed-rpc object into JavaScript error', function() {
      var axed = new AxeService(baseConfig);
      var error = axed._wrapRPCError({message: 'Test error', code: -1});
  ...
  describe('#_initChain', function() {
    ...
    it('will set height and genesis buffer', function(done) {
      ...
      axed.getRawBlock = sinon.stub().callsArgWith(1, null, genesisBuffer);
      axed.nodes.push({
        client: {
      ...
      axed._initChain(function() {
        log.info.callCount.should.equal(1);
        axed.getRawBlock.callCount.should.equal(1);
        axed.getRawBlock.args[0][0].should.equal('genesishash');
        axed.height.should.equal(5000);
        axed.genesisBuffer.should.equal(genesisBuffer);
        done();
      });
    });
    ...
    it('it will handle error from getRawBlock', function(done) {
      ...
      axed.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
      axed._initChain(function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
  ...
  describe('#_getDefaultConf', function() {
    afterEach(function() {
      axecore.Networks.disableRegtest();
      baseConfig.node.network = axecore.Networks.testnet;
    });
    it('will get default rpc port for livenet', function() {
      ...
      var axed = new AxeService(config);
      axed._getDefaultConf().rpcport.should.equal(9998);
    });
    it('will get default rpc port for testnet', function() {
      ...
      var axed = new AxeService(config);
      axed._getDefaultConf().rpcport.should.equal(19998);
    });
  ...
  describe('#_getNetworkConfigPath', function() {
    ...
    it('will get default config path for livenet', function() {
      ...
      var axed = new AxeService(config);
      should.equal(axed._getNetworkConfigPath(), undefined);
    });
    it('will get default rpc port for testnet', function() {
      var config = {
        node: {
      ...
      var axed = new AxeService(config);
      axed._getNetworkConfigPath().should.equal('testnet3/axe.conf');
    });
    it('will get default rpc port for regtest', function() {
      axecore.Networks.enableRegtest();
      var config = {
      ...
      var axed = new AxeService(config);
      axed._getNetworkConfigPath().should.equal('regtest/axe.conf');
    });
  });
  ...
  describe('#_getNetworkOption', function() {
    ...
    it('return --testnet for testnet', function() {
      var axed = new AxeService(baseConfig);
      axed.node.network = axecore.Networks.testnet;
      axed._getNetworkOption().should.equal('--testnet');
    });
    it('return --regtest for testnet', function() {
      var axed = new AxeService(baseConfig);
      axed.node.network = axecore.Networks.testnet;
      axecore.Networks.enableRegtest();
      axed._getNetworkOption().should.equal('--regtest');
    });
    it('return undefined for livenet', function() {
      var axed = new AxeService(baseConfig);
      axed.node.network = axecore.Networks.livenet;
      axecore.Networks.enableRegtest();
      should.equal(axed._getNetworkOption(), undefined);
    });
  });
  ...
  describe('#_zmqBlockHandler', function() {
    it('will emit block', function(done) {
      ...
      axed._rapidProtectedUpdateTip = sinon.stub();
      axed.on('block', function(block) {
        block.should.equal(message);
        done();
      });
      axed._zmqBlockHandler(node, message);
    });
    ...
    it('will call function to update tip', function() {
      ...
      axed._rapidProtectedUpdateTip = sinon.stub();
      axed._zmqBlockHandler(node, message);
      axed._rapidProtectedUpdateTip.callCount.should.equal(1);
      axed._rapidProtectedUpdateTip.args[0][0].should.equal(node);
      axed._rapidProtectedUpdateTip.args[0][1].should.equal(message);
    });
    it('will emit to subscribers', function(done) {
      ...
      axed._rapidProtectedUpdateTip = sinon.stub();
      var emitter = new EventEmitter();
      axed.subscriptions.hashblock.push(emitter);
      emitter.on('axed/hashblock', function(blockHash) {
  ...
  describe('#_rapidProtectedUpdateTip', function() {
    it('will limit tip updates with rapid calls', function(done) {
      ...
      var count = 0;
      function repeat() {
        axed._rapidProtectedUpdateTip(node, message);
        count++;
        if (count < 50) {
          repeat();
        }
  ...
  describe('#_updateTip', function() {
    ...
    it('log and emit rpc error from get block', function(done) {
      var axed = new AxeService(baseConfig);
      axed.syncPercentage = sinon.stub();
      ...
      axed._updateTip(node, message);
    });
    it('emit synced if percentage is 100', function(done) {
      var axed = new AxeService(baseConfig);
      axed.syncPercentage = sinon.stub().callsArgWith(0, null, 100);
      axed.on('synced', function() {
        done();
      });
      var node = {
        client: {
    ...
    it('NOT emit synced if percentage is less than 100', function(done) {
      var axed = new AxeService(baseConfig);
      axed.syncPercentage = sinon.stub().callsArgWith(0, null, 99);
      axed.on('synced', function() {
    ...
    it('log and emit error from syncPercentage', function(done) {
      var axed = new AxeService(baseConfig);
      axed.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
      axed.on('error', function(err) {
        log.error.callCount.should.equal(1);
        err.message.should.equal('test');
        done();
    ...
    it('reset caches and set height', function(done) {
      var axed = new AxeService(baseConfig);
      axed.syncPercentage = sinon.stub();
      axed._resetCaches = sinon.stub();
      axed.on('tip', function(height) {
        axed._resetCaches.callCount.should.equal(1);
        height.should.equal(10);
        axed.height.should.equal(10);
        done();
      });
      var node = {
        client: {
    ...
    it('will not call syncPercentage if node is stopping', function(done) {
      ...
      axed.on('tip', function() {
        axed.syncPercentage.callCount.should.equal(0);
        done();
      });
      axed._updateTip(node, message);
    });
  ...
  describe('#_getAddressesFromTransaction', function() {
    it('will get results using axecore.Transaction', function() {
      var axed = new AxeService(baseConfig);
      var wif = 'XGLgPK8gbmzU7jcbw34Pj55AXV7SmG6carKuiwtu4WtvTjyTbpwX';
      var privkey = axecore.PrivateKey.fromWIF(wif);
      var inputAddress = privkey.toAddress(axecore.Networks.testnet);
      var outputAddress = axecore.Address('8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi');
      var tx = axecore.Transaction();
      tx.from({
        txid: '4a5e1e4baab89f3a32518a88c31bc87f618f76673e2cc77ab2127b7afdeda33b',
        outputIndex: 0,
        script: axecore.Script(inputAddress),
        address: inputAddress.toString(),
        satoshis: 5000000000
      });
      tx.to(outputAddress, 5000000000);
      tx.sign(privkey);
      var addresses = axed._getAddressesFromTransaction(tx);
    ...
    it('will handle non-standard script types', function() {
      ...
      tx.addOutput(axecore.Transaction.Output({
        script: axecore.Script('OP_TRUE'),
        satoshis: 5000000000
      }));
      var addresses = axed._getAddressesFromTransaction(tx);
    ...
    it('will return unique values', function() {
      var axed = new AxeService(baseConfig);
      var tx = axecore.Transaction();
      var address = axecore.Address('8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi');
      tx.addOutput(axecore.Transaction.Output({
        script: axecore.Script(address),
        satoshis: 5000000000
      }));
      tx.addOutput(axecore.Transaction.Output({
        script: axecore.Script(address),
  ...
  describe('#_notifyAddressTxidSubscribers', function() {
    it('will emit event if matching addresses', function(done) {
      ...
      axed._getAddressesFromTransaction = sinon.stub().returns([address]);
      var emitter = new EventEmitter();
      axed.subscriptions.address[address] = [emitter];
      var txid = '46f24e0c274fc07708b781963576c4c5d5625d926dbb0a17fa865dcd9fe58ea0';
      var transaction = {};
      emitter.on('axed/addresstxid', function(data) {
        data.address.should.equal(address);
        data.txid.should.equal(txid);
        done();
      });
      sinon.spy(emitter, 'emit');
      axed._notifyAddressTxidSubscribers(txid, transaction);
  ...
  describe('#_zmqTransactionHandler', function() {
    it('will emit to subscribers', function(done) {
      ...
      var emitter = new EventEmitter();
      axed.subscriptions.rawtransaction.push(emitter);
      emitter.on('axed/rawtransaction', function(hex) {
        hex.should.be.a('string');
        hex.should.equal(expectedBuffer.toString('hex'));
        done();
      });
      var node = {};
      axed._zmqTransactionHandler(node, expectedBuffer);
    });
    it('will NOT emit to subscribers more than once for the same tx', function(done) {
      ...
      var emitter = new EventEmitter();
      axed.subscriptions.rawtransaction.push(emitter);
      emitter.on('axed/rawtransaction', function() {
        done();
      });
      var node = {};
      axed._zmqTransactionHandler(node, expectedBuffer);
  ...
  describe('#_zmqTransactionLockHandler', function() {
    it('will emit to subscribers', function(done) {
      ...
      var expectedBuffer = new Buffer(txhex, 'hex');
      var emitter = new EventEmitter();
      axed.subscriptions.transactionlock.push(emitter);
      emitter.on('axed/transactionlock', function(hex) {
        hex.should.be.a('string');
        hex.should.equal(expectedBuffer.toString('hex'));
        done();
      });
      var node = {};
      axed._zmqTransactionLockHandler(node, expectedBuffer);
    });
    it('will NOT emit to subscribers more than once for the same tx', function(done) {
      var axed = new AxeService(baseConfig);
      var expectedBuffer = new Buffer(txhex, 'hex');
      var emitter = new EventEmitter();
      axed.subscriptions.transactionlock.push(emitter);
      emitter.on('axed/transactionlock', function() {
        done();
      });
      var node = {};
      axed._zmqTransactionLockHandler(node, expectedBuffer);
    ...
    it('will emit "tx" event', function(done) {
      var axed = new AxeService(baseConfig);
      var expectedBuffer = new Buffer(txhex, 'hex');
      axed.on('txlock', function(buffer) {
        buffer.should.be.instanceof(Buffer);
        buffer.toString('hex').should.equal(expectedBuffer.toString('hex'));
        done();
      });
  ...
  describe('#_checkSyncedAndSubscribeZmqEvents', function() {
    ...
    it('log errors, update tip and subscribe to zmq events', function(done) {
      var axed = new AxeService(baseConfig);
      axed._updateTip = sinon.stub();
      axed._subscribeZmqEvents = sinon.stub();
      var blockEvents = 0;
      axed.on('block', function() {
        blockEvents++;
      });
      var getBestBlockHash = sinon.stub().callsArgWith(0, null, {
      ...
      axed._checkSyncedAndSubscribeZmqEvents(node);
      setTimeout(function() {
        log.error.callCount.should.equal(2);
        blockEvents.should.equal(11);
        axed._updateTip.callCount.should.equal(11);
        axed._subscribeZmqEvents.callCount.should.equal(1);
        done();
      }, 200);
    });
  ...
  describe('#_subscribeZmqEvents', function() {
    ...
    it('will call relevant handler for rawtx topics', function(done) {
      var axed = new AxeService(baseConfig);
      axed._zmqTransactionHandler = sinon.stub();
      var node = {
      ...
      node.zmqSubSocket.on('message', function() {
        axed._zmqTransactionHandler.callCount.should.equal(1);
        done();
      });
    ...
    it('will call relevant handler for hashblock topics', function(done) {
      var axed = new AxeService(baseConfig);
      axed._zmqBlockHandler = sinon.stub();
      var node = {
        zmqSubSocket: new EventEmitter()
      };
      node.zmqSubSocket.subscribe = sinon.stub();
      axed._subscribeZmqEvents(node);
      node.zmqSubSocket.on('message', function() {
        axed._zmqBlockHandler.callCount.should.equal(1);
        done();
      });
    ...
    it('will ignore unknown topic types', function(done) {
      ...
      axed._subscribeZmqEvents(node);
      node.zmqSubSocket.on('message', function() {
        axed._zmqBlockHandler.callCount.should.equal(0);
        axed._zmqTransactionHandler.callCount.should.equal(0);
        done();
      });
  ...
  describe('#_initZmqSubSocket', function() {
    it('will setup zmq socket', function() {
      ...
      var axed = new AxeService(baseConfig);
      var node = {};
      axed._initZmqSubSocket(node, 'url');
  ...
  describe('#_checkReindex', function() {
    ...
    it('give error from client getblockchaininfo', function(done) {
      ...
      axed._checkReindex(node, function(err) {
        should.exist(err);
    ...
    it('will wait until sync is 100 percent', function(done) {
      ...
      axed._checkReindex(node, function() {
  ...
  describe('#_loadTipFromNode', function() {
    ...
    it('will give rpc from client getbestblockhash', function(done) {
      ...
      axed._loadTipFromNode(node, function(err) {
        err.should.be.instanceof(Error);
  ...
  describe('#_stopSpawnedProcess', function() {
    ...
    it('it will kill process and resume', function(done) {
      ...
      var TestAxeService = proxyquire('../../lib/services/axed', {
        fs: {
          readFile: readFile
        }
      });
      var axed = new TestAxeService(baseConfig);
      axed.spawnStopTime = 1;
      axed._process = {};
      axed._process.kill = sinon.stub();
      axed._stopSpawnedAxe(function(err) {
        if (err) {
          return done(err);
        }
        axed._process.kill.callCount.should.equal(1);
        log.warn.callCount.should.equal(1);
        done();
      });
    });
    it('it will attempt to kill process and resume', function(done) {
      ...
      error2.code = 'ESRCH';
      axed._process.kill = sinon.stub().throws(error2);
      axed._stopSpawnedAxe(function(err) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_spawnChildProcess', function() {
    ...
    it('will give error from spawn config', function(done) {
      var axed = new AxeService(baseConfig);
      axed._loadSpawnConfiguration = sinon.stub();
      axed._loadSpawnConfiguration = sinon.stub().throws(new Error('test'));
      axed._spawnChildProcess(function(err) {
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
      });
    });
    it('will give error from stopSpawnedAxe', function() {
      var axed = new AxeService(baseConfig);
      axed._loadSpawnConfiguration = sinon.stub();
      axed._stopSpawnedAxe = sinon.stub().callsArgWith(0, new Error('test'));
      axed._spawnChildProcess(function(err) {
        err.should.be.instanceOf(Error);
        err.message.should.equal('test');
      });
    ...
    it('will exit spawn if shutdown', function() {
      ...
      var axed = new TestAxeService(config);
      axed.spawn = {};
      axed._loadSpawnConfiguration = sinon.stub();
      axed._stopSpawnedAxe = sinon.stub().callsArgWith(0, null);
      axed.node.stopping = true;
      axed._spawnChildProcess(function(err) {
        err.should.be.instanceOf(Error);
    ...
    it('will include network with spawn command and init zmq/rpc on node', function(done) {
      ...
      axed._loadSpawnConfiguration = sinon.stub();
      axed.spawn = {};
      axed.spawn.exec = 'testexec';
      axed.spawn.configPath = 'testdir/axe.conf';
      axed.spawn.datadir = 'testdir';
      axed.spawn.config = {};
      axed.spawn.config.rpcport = 20001;
      axed.spawn.config.rpcuser = 'axe';
      axed.spawn.config.rpcpassword = 'password';
      axed.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
      axed.spawn.config.zmqpubrawtxlock = 'tcp://127.0.0.1:30001';

      axed._loadTipFromNode = sinon.stub().callsArgWith(1, null);
      axed._initZmqSubSocket = sinon.stub();
      axed._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      axed._checkReindex = sinon.stub().callsArgWith(1, null);
      axed._spawnChildProcess(function(err, node) {
        should.not.exist(err);
        spawn.callCount.should.equal(1);
        spawn.args[0][0].should.equal('testexec');
        spawn.args[0][1].should.deep.equal([
          '--conf=testdir/axe.conf',
          '--datadir=testdir',
          '--testnet'
        ]);
        ...
        axed._loadTipFromNode.callCount.should.equal(1);
        axed._initZmqSubSocket.callCount.should.equal(1);
        should.exist(axed._initZmqSubSocket.args[0][0].client);
        axed._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
        axed._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
        should.exist(axed._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
        should.exist(node);
        should.exist(node.client);
        done();
      });
    });
    it('will respawn axed spawned process', function(done) {
      var process = new EventEmitter();
      var spawn = sinon.stub().returns(process);
      var TestAxeService = proxyquire('../../lib/services/axed', {
        fs: {
      ...
      axed._loadSpawnConfiguration = sinon.stub();
      axed.spawn = {};
      axed.spawn.exec = 'axed';
      axed.spawn.datadir = '/tmp/axe';
      axed.spawn.configPath = '/tmp/axe/axe.conf';
      axed.spawn.config = {};
      axed.spawnRestartTime = 1;
      axed._loadTipFromNode = sinon.stub().callsArg(1);
      axed._initZmqSubSocket = sinon.stub();
      axed._checkReindex = sinon.stub().callsArg(1);
      axed._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      axed._stopSpawnedAxe = sinon.stub().callsArg(0);
      sinon.spy(axed, '_spawnChildProcess');
      axed._spawnChildProcess(function(err) {
        if (err) {
          return done(err);
        }
        process.once('exit', function() {
          setTimeout(function() {
            axed._spawnChildProcess.callCount.should.equal(2);
            done();
          }, 5);
        });
        process.emit('exit', 1);
    ...
    it('will emit error during respawn', function(done) {
      ...
      axed._spawnChildProcess(function(err) {
        ...
        axed._spawnChildProcess = sinon.stub().callsArgWith(0, new Error('test'));
        axed.on('error', function(err) {
          err.should.be.instanceOf(Error);
          err.message.should.equal('test');
          done();
    ...
    it('will NOT respawn axed spawned process if shutting down', function(done) {
      var process = new EventEmitter();
      var spawn = sinon.stub().returns(process);
      var TestAxeService = proxyquire('../../lib/services/axed', {
        fs: {
      ...
      axed._spawnChildProcess(function(err) {
        ...
        process.once('exit', function() {
          setTimeout(function() {
            axed._spawnChildProcess.callCount.should.equal(1);
            done();
          }, 5);
        });
        process.emit('exit', 1);
    ...
    it('will give error after 60 retries', function(done) {
      ...
      axed.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
      axed.spawn.config.zmqpubrawtxlock = 'tcp://127.0.0.1:30001';
      axed._loadTipFromNode = sinon.stub().callsArgWith(1, new Error('test'));
      axed._spawnChildProcess(function(err) {
        axed._loadTipFromNode.callCount.should.equal(60);
        err.should.be.instanceof(Error);
        done();
      });
    });
    it('will give error from check reindex', function(done) {
      ...
      axed._initZmqSubSocket = sinon.stub();
      axed._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      axed._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
  ...
  describe('#_connectProcess', function() {
    it('will give error if connecting while shutting down', function(done) {
      ...
      var axed = new AxeService(config);
      axed.node.stopping = true;
      axed.startRetryInterval = 100;
      axed._loadTipFromNode = sinon.stub();
      axed._connectProcess({}, function(err) {
        err.should.be.instanceof(Error);
        err.message.should.match(/Stopping while trying to connect/);
        axed._loadTipFromNode.callCount.should.equal(0);
        done();
      });
    });
    it('will give error from loadTipFromNode after 60 retries', function(done) {
      ...
      axed.startRetryInterval = 1;
      var config = {};
      axed._connectProcess(config, function(err) {
        err.should.be.instanceof(Error);
        axed._loadTipFromNode.callCount.should.equal(60);
        done();
      });
    ...
    it('will init zmq/rpc on node', function(done) {
      ...
      axed._loadTipFromNode = sinon.stub().callsArgWith(1, null);
      var config = {};
      axed._connectProcess(config, function(err, node) {
        should.not.exist(err);
        axed._loadTipFromNode.callCount.should.equal(1);
        axed._initZmqSubSocket.callCount.should.equal(1);
        axed._loadTipFromNode.callCount.should.equal(1);
  ...
  describe('#start', function() {
    ...
    it('will give error if "spawn" and "connect" are both not configured', function(done) {
      ...
      axed.start(function(err) {
        err.should.be.instanceof(Error);
        err.message.should.match(/Axe configuration options/);
      });
      done();
    });
    ...
    it('will give error from connectProcess', function(done) {
      var axed = new AxeService(baseConfig);
      axed._connectProcess = sinon.stub().callsArgWith(1, new Error('test'));
      axed.options = {
        connect: [
          {}
        ]
      ...
      axed.start(function(err) {
        axed._connectProcess.callCount.should.equal(1);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will push node from spawnChildProcess', function(done) {
      var axed = new AxeService(baseConfig);
      var node = {};
      axed._initChain = sinon.stub().callsArg(0);
      axed._spawnChildProcess = sinon.stub().callsArgWith(0, null, node);
      axed.options = {
        spawn: {}
      };
      axed.start(function(err) {
        should.not.exist(err);
        axed.nodes.length.should.equal(1);
        done();
      });
    });
    it('will push node from connectProcess', function(done) {
      ...
      axed._initChain = sinon.stub().callsArg(0);
      var nodes = [{}];
      axed._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
      axed.options = {
        connect: [
          {}
        ]
  ...
  describe('#isSynced', function() {
    ...
    it('will give "true" if percentage is 100.00', function(done) {
      var axed = new AxeService(baseConfig);
      axed.syncPercentage = sinon.stub().callsArgWith(0, null, 100.00);
      axed.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "true" if percentage is 99.98', function(done) {
      var axed = new AxeService(baseConfig);
      axed.syncPercentage = sinon.stub().callsArgWith(0, null, 99.98);
      axed.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "false" if percentage is 99.49', function(done) {
      var axed = new AxeService(baseConfig);
      axed.syncPercentage = sinon.stub().callsArgWith(0, null, 99.49);
      axed.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "false" if percentage is 1', function(done) {
      var axed = new AxeService(baseConfig);
      axed.syncPercentage = sinon.stub().callsArgWith(0, null, 1);
      axed.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
  ...
  describe('#syncPercentage', function() {
    it('will give rpc error', function(done) {
      ...
      axed.syncPercentage(function(err) {
        should.exist(err);
    ...
    it('will call client getInfo and give result', function(done) {
      ...
      axed.syncPercentage(function(err, percentage) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_normalizeAddressArg', function() {
    it('will turn single address into array', function() {
      var axed = new AxeService(baseConfig);
      var args = axed._normalizeAddressArg('address');
      args.should.deep.equal(['address']);
    });
    it('will keep an array as an array', function() {
      var axed = new AxeService(baseConfig);
      var args = axed._normalizeAddressArg(['address', 'address']);
  ...
  describe('#getAddressBalance', function() {
    it('will give rpc error', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      var options = {};
      axed.getAddressBalance(address, options, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
    it('will give balance', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      var options = {};
      axed.getAddressBalance(address, options, function(err, data) {
        if (err) {
          return done(err);
        }
        data.balance.should.equal(10000);
        data.received.should.equal(100000);
        axed.getAddressBalance(address, options, function(err, data2) {
          if (err) {
            return done(err);
          }
  ...
  describe('#getAddressUnspentOutputs', function() {
    it('will give rpc error', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      axed.getAddressUnspentOutputs(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will give results from client getaddressutxos', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      axed.getAddressUnspentOutputs(address, options, function(err, utxos) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_getBalanceFromMempool', function() {
    it('will sum satoshis', function() {
      ...
      var sum = axed._getBalanceFromMempool(deltas);
  ...
  describe('#_getTxidsFromMempool', function() {
    it('will filter to txids', function() {
      ...
      var txids = axed._getTxidsFromMempool(deltas);
  ...
  describe('#_getHeightRangeQuery', function() {
    it('will detect range query', function() {
      ...
      var rangeQuery = axed._getHeightRangeQuery(options);
    ...
    it('will get range properties', function() {
      ...
      var clone = {};
      axed._getHeightRangeQuery(options, clone);
    ...
    it('will throw error with invalid range', function() {
      ...
      (function() {
        axed._getHeightRangeQuery(options);
  ...
  describe('#getAddressTxids', function() {
    it('will give error from _getHeightRangeQuery', function(done) {
      var axed = new AxeService(baseConfig);
      axed._getHeightRangeQuery = sinon.stub().throws(new Error('test'));
      axed.getAddressTxids('address', {}, function(err) {
        err.should.be.instanceOf(Error);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give rpc error from mempool query', function() {
      ...
      var options = {};
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      axed.getAddressTxids(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
      });
    });
    ...
    it('will get txid results', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      axed.getAddressTxids(address, options, function(err, txids) {
        if (err) {
          return done(err);
        }
    ...
    it('will get txid results from cache and live mempool', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      axed.getAddressTxids(address, {queryMempool: false}, function(err, txids) {
        if (err) {
          return done(err);
        }
        getAddressTxids.callCount.should.equal(1);
        ...
        axed.getAddressTxids(address, {queryMempool: true}, function(err, txids) {
          if (err) {
            return done(err);
          }
          getAddressTxids.callCount.should.equal(1);
  ...
  describe('#_getConfirmationDetail', function() {
    ...
    it('should get 0 confirmation', function() {
      ...
      var axed = new AxeService(baseConfig);
      axed.height = 10;
      var confirmations = axed._getConfirmationsDetail(tx);
  ...
  describe('#_getAddressDetailsForInput', function() {
    it('will return if missing an address', function() {
      var axed = new AxeService(baseConfig);
      var result = {};
      axed._getAddressDetailsForInput({}, 0, result, []);
    ...
    it('will only add address if it matches', function() {
      var axed = new AxeService(baseConfig);
      var result = {};
      axed._getAddressDetailsForInput({
        address: 'address1'
      }, 0, result, ['address2']);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
  ...
  describe('#_getAddressDetailsForOutput', function() {
    it('will return if missing an address', function() {
      var axed = new AxeService(baseConfig);
      var result = {};
      axed._getAddressDetailsForOutput({}, 0, result, []);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
    });
    it('will only add address if it matches', function() {
      var axed = new AxeService(baseConfig);
      var result = {};
      axed._getAddressDetailsForOutput({
        address: 'address1'
      }, 0, result, ['address2']);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
  ...
  describe('#_getAddressDetailsForTransaction', function() {
    it('will calculate details for the transaction', function(done) {
      ...
      var details = axed._getAddressDetailsForTransaction(tx, addresses);
  ...
  describe('#_getAddressDetailedTransaction', function() {
    it('will get detailed transaction info', function(done) {
      ...
      var axed = new AxeService(baseConfig);
      axed.getDetailedTransaction = sinon.stub().callsArgWith(1, null, tx);
      axed.height = 300;
      var addresses = {};
      axed._getAddressDetailsForTransaction = sinon.stub().returns({
        addresses: addresses,
        satoshis: 1000,
      });
      axed._getAddressDetailedTransaction(txid, {}, function(err, details) {
        if (err) {
          return done(err);
        }
    ...
    it('give error from getDetailedTransaction', function(done) {
      var txid = '46f24e0c274fc07708b781963576c4c5d5625d926dbb0a17fa865dcd9fe58ea0';
      var axed = new AxeService(baseConfig);
      axed.getDetailedTransaction = sinon.stub().callsArgWith(1, new Error('test'));
      axed._getAddressDetailedTransaction(txid, {}, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
  ...
  describe('#_getAddressStrings', function() {
    it('will get address strings from axecore addresses', function() {
      var addresses = [
        axecore.Address('XjxDQFjTNEP9dcrJhBLvy5i1Dobz4x1LJN'),
        axecore.Address('7d5169eBcGHF4BYC6DTffTyeCpWbrZnNgz'),
      ];
      var axed = new AxeService(baseConfig);
      var strings = axed._getAddressStrings(addresses);
    ...
    it('will give error with unknown', function() {
      ...
      var axed = new AxeService(baseConfig);
      (function() {
        axed._getAddressStrings(addresses);
  ...
  describe('#getAddressHistory', function() {
    ...
    it('will give error with "from" and "to" range that exceeds max size', function(done) {
      var axed = new AxeService(baseConfig);
      axed.getAddressHistory(address, {from: 0, to: 51}, function(err) {
        should.exist(err);
    ...
    it('will give error with "from" and "to" order is reversed', function(done) {
      var axed = new AxeService(baseConfig);
      axed.getAddressTxids = sinon.stub().callsArgWith(2, null, []);
      axed.getAddressHistory(address, {from: 51, to: 0}, function(err) {
        should.exist(err);
        err.message.match(/^\"from/);
        done();
      });
    ...
    it('will give error from _getAddressDetailedTransaction', function(done) {
      var axed = new AxeService(baseConfig);
      axed.getAddressTxids = sinon.stub().callsArgWith(2, null, ['txid']);
      axed._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
      axed.getAddressHistory(address, {}, function(err) {
        should.exist(err);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give an error if length of addresses is too long', function(done) {
      ...
      var axed = new AxeService(baseConfig);
      axed.maxAddressesQuery = 100;
      axed.getAddressHistory(addresses, {}, function(err) {
        should.exist(err);
    ...
    it('give error from getAddressTxids', function(done) {
      var axed = new AxeService(baseConfig);
      axed.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
      axed.getAddressHistory('address', {}, function(err) {
        should.exist(err);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
    ...
    it('will paginate', function(done) {
      var axed = new AxeService(baseConfig);
      axed._getAddressDetailedTransaction = function(txid, options, callback) {
        callback(null, txid);
      };
      var txids = ['one', 'two', 'three', 'four'];
      axed.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
      axed.getAddressHistory('address', {from: 1, to: 3}, function(err, data) {
        if (err) {
          return done(err);
        }
  ...
  describe('#getAddressSummary', function() {
    ...
    it('will handle error from getAddressTxids', function(done) {
      ...
      var address = '';
      var options = {};
      axed.getAddressSummary(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
    ...
    it('will handle error from getAddressBalance', function(done) {
      ...
      axed.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
      axed.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
      var address = '';
      var options = {};
      axed.getAddressSummary(address, options, function(err) {
        should.exist(err);
    ...
    it('will get from cache with noTxList', function(done) {
      ...
      axed.getAddressSummary(address, options, function(err, summary) {
        ...
        axed.getAddressTxids.callCount.should.equal(1);
        axed.getAddressBalance.callCount.should.equal(1);
        axed.getAddressSummary(address, options, function(err, summary) {
          checkSummary(summary);
          axed.getAddressTxids.callCount.should.equal(1);
          axed.getAddressBalance.callCount.should.equal(1);
    ...
    it('will skip querying the mempool with queryMempool set to false', function(done) {
      ...
      axed.getAddressSummary(address, options, function() {
        getAddressMempool.callCount.should.equal(0);
        done();
      });
    });
  ...
  describe('#getRawBlock', function() {
    ...
    it('will give rcp error from client getblockhash', function(done) {
      ...
      axed.getRawBlock(10, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will give rcp error from client getblock', function(done) {
      ...
      axed.getRawBlock(blockhash, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes for getblock', function(done) {
      ...
      axed.nodesIndex = 2;
      axed.getRawBlock(blockhash, function(err, buffer) {
        if (err) {
          return done(err);
        }
        buffer.should.be.instanceof(Buffer);
    ...
    it('will get block by height', function(done) {
      ...
      axed.getRawBlock(0, function(err, buffer) {
        if (err) {
          return done(err);
        }
        buffer.should.be.instanceof(Buffer);
  ...
  describe('#getBlock', function() {
    ...
    it('will getblock as axecore object from height', function(done) {
      var axed = new AxeService(baseConfig);
      var getBlock = sinon.stub().callsArgWith(2, null, {
        result: blockhex
      });
      ...
      axed.getBlock(0, function(err, block) {
        should.not.exist(err);
        ...
        block.should.be.instanceof(axecore.Block);
        done();
      });
    });
    it('will getblock as axecore object', function(done) {
      var axed = new AxeService(baseConfig);
      var getBlock = sinon.stub().callsArgWith(2, null, {
        result: blockhex
      });
      ...
      axed.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
        should.not.exist(err);
  ...
  describe('#getBlockHashesByTimestamp', function() {
    it('should give an rpc error', function(done) {
      ...
      axed.getBlockHashesByTimestamp(1441911000, 1441914000, function(err, hashes) {
        should.exist(err);
    ...
    it('should get the correct block hashes', function(done) {
      ...
      axed.getBlockHashesByTimestamp(1441914000, 1441911000, function(err, hashes) {
        should.not.exist(err);
  ...
  describe('#getBlockHeader', function() {
    ...
    it('will give error from getBlockHash', function() {
      ...
      axed.getBlockHeader(10, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('it will give rpc error from client getblockheader', function() {
      ...
      axed.getBlockHeader(blockhash, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('it will give rpc error from client getblockhash', function() {
      ...
      axed.getBlockHeader(0, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('will give result from client getblockheader (from height)', function() {
      ...
      axed.getBlockHeader(0, function(err, blockHeader) {
        should.not.exist(err);
    ...
    it('will give result from client getblockheader (from hash)', function() {
      ...
      axed.getBlockHeader(blockhash, function(err, blockHeader) {
        should.not.exist(err);
        getBlockHash.callCount.should.equal(0);
        blockHeader.should.deep.equal(result);
      });
  ...
  describe('#getBlockHeaders', function(){
      ...
      it('will gave error from getBlockHash', function(){
          ...
          axed.getBlockHeaders(10, function(err) {
              err.should.be.instanceof(Error);
          });
      });
      it('it will give rpc error from client getblockheaders', function() {
          ...
          axed.getBlockHeaders(blockhash, function(err){
              err.should.be.instanceof(Error);
          });
      });
      it("will get an array of block headers", function(){
          ...
          axed.getBlockHeaders(_blockHash, function(err, blockHeader){
              should.not.exist(err);
  ...
  describe('#_maybeGetBlockHash', function() {
    it('will not get block hash with an address', function(done) {
      ...
      axed._maybeGetBlockHash('8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi', function(err, hash) {
        if (err) {
          return done(err);
        }
        getBlockHash.callCount.should.equal(0);
    ...
    it('will not get block hash with non zero-nine numeric string', function(done) {
      ...
      axed._maybeGetBlockHash('109a', function(err, hash) {
        if (err) {
          return done(err);
        }
        getBlockHash.callCount.should.equal(0);
    ...
    it('will get the block hash if argument is a number', function(done) {
      ...
      axed._maybeGetBlockHash(10, function(err, hash) {
        if (err) {
          return done(err);
        }
  ...
  describe('#getBlockOverview', function() {
    ...
    it('will handle error from maybeGetBlockHash', function(done) {
      var axed = new AxeService(baseConfig);
      axed._maybeGetBlockHash = sinon.stub().callsArgWith(1, new Error('test'));
      axed.getBlockOverview(blockhash, function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
    });
    ...
    it('will give expected result', function(done) {
      ...
      axed.getBlockOverview(blockhash, function(err, blockOverview) {
        if (err) {
          return done(err);
        }
  ...
  describe('#estimateFee', function() {
    it('will give rpc error', function(done) {
      ...
      axed.estimateFee(1, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client estimateFee and give result', function(done) {
      ...
      axed.estimateFee(1, function(err, feesPerKb) {
        if (err) {
          return done(err);
        }
  ...
  describe('#sendTransaction', function(done) {
    var tx = axecore.Transaction(txhex);
    it('will give rpc error', function() {
      ...
      axed.sendTransaction(txhex, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
      });
    });
    it('will send to client and get hash', function() {
      ...
      axed.sendTransaction(txhex, function(err, hash) {
        if (err) {
          return done(err);
        }
    ...
    it('will send to client with absurd fees and get hash', function() {
      ...
      axed.sendTransaction(txhex, {allowAbsurdFees: true}, function(err, hash) {
        if (err) {
          return done(err);
        }
        hash.should.equal(tx.hash);
    ...
    it('missing callback will throw error', function() {
      ...
      var transaction = axecore.Transaction();
      (function() {
        axed.sendTransaction(transaction);
      }).should.throw(Error);
    });
  });
  ...
  describe('#getRawTransaction', function() {
    it('will give rpc error', function(done) {
      ...
      axed.getRawTransaction('txid', function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes', function(done) {
      ...
      axed.getRawTransaction('txid', function(err, tx) {
        if (err) {
          return done(err);
        }
        should.exist(tx);
  ...
  describe('#getTransaction', function() {
    it('will give rpc error', function(done) {
      ...
      axed.getTransaction('txid', function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes', function(done) {
      ...
      axed.getTransaction('txid', function(err, tx) {
        if (err) {
          return done(err);
        }
        should.exist(tx);
        tx.should.be.an.instanceof(axecore.Transaction);
        done();
      });
    });
    it('will get from cache', function(done) {
  ...
  describe('#getDetailedTransaction', function() {
    ...
    it('should give a transaction with height and timestamp', function(done) {
      ...
      axed.getDetailedTransaction(txid, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('should give a transaction with all properties', function(done) {
      ...
      axed.getDetailedTransaction(txid, function(err, tx) {
        if (err) {
          return done(err);
        }
        checkTx(tx);
  ...
  describe('#getBestBlockHash', function() {
    it('will give rpc error', function(done) {
      ...
      axed.getBestBlockHash(function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client getInfo and give result', function(done) {
      ...
      axed.getBestBlockHash(function(err, hash) {
        if (err) {
          return done(err);
        }
        should.exist(hash);
  ...
  describe('#getSpentInfo', function() {
    it('will give rpc error', function(done) {
      ...
      axed.getSpentInfo({}, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will empty object when not found', function(done) {
      ...
      axed.getSpentInfo({}, function(err, info) {
        should.not.exist(err);
  ...
  describe('#govObject', function() {
    it('will call client gobject list and give result', function(done) {
        ...
        axed.govObjectList({type: 1}, function(err, result) {
            if (err) {
                return done(err);
            }
            should.exist(result);
    ...
    it('will call client gobject get and give result', function(done) {
      ...
      var gobject = sinon.stub().callsArgWith(2, null, {
        result: {
          "DataHex": "",
          "DataString": "[[\"proposal\",{\"end_epoch\":1483855193,\"name\":\"7ea9acff5ae83d8c9e217d30a2b4d10ef8f171f8\",\"payment_address\":\"yZ7DYkDHCHfH1dw7rKdYaKcVykZmunbqNL\",\"payment_amount\":84,\"start_epoch\":1483765282,\"type\":1,\"url\":\"ttps://www.dash.org\"}]]",
          "Hash": "4ef24027c631c43035aa4cf5c672e1298311decd9cffbd16731f454c9c0d6d00",
      ...
      axed.govObjectHash('4ef24027c631c43035aa4cf5c672e1298311decd9cffbd16731f454c9c0d6d00', function(err, result) {
        if (err) {
          return done(err);
        }
        ...
        should.equal(DataObject.url, 'ttps://www.dash.org');
        done();
      });
    });
	...
	describe('#sporksList', function(){
		it('will call client sporks and give result', function(done){
			...
			axed.getSpork(function(err, SporkList) {
				if (err) {
					return done(err);
				}
  ...
  describe('#getMNList', function(){
    it('will call client masternode list and give result', function(done){
	    var axed = new AxeService(baseConfig);
	    axed.isSynced = function(callback) { return callback(null, true) };
	    axed.nodes.push({
		    client: {
    ...
    it('will return error if one of nodes not synced yet', function(done){
      var axed = new AxeService(baseConfig);
      axed.isSynced = function(callback) { return callback(null, false) };
      axed.nodes.push({
        client: {
          masternodelist: function(type, cb){
            switch (type){
    ...
    it('will return error if checking synced state of nodes failed', function(done){
      var axed = new AxeService(baseConfig);
      axed.isSynced = function(callback) { return callback(new Error('Failed')) };
      axed.nodes.push({
        client: {
          masternodelist: function(type, cb){
            switch (type){
  ...
  describe('#generateBlock', function() {
    it('will give rpc error', function(done) {
      ...
      axed.generateBlock(10, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client generate and give result', function(done) {
      ...
      axed.generateBlock(10, function(err, hashes) {
        if (err) {
          return done(err);
        }
  ...
  describe('#stop', function() {
    ...
    it('will exit spawned process', function(done) {
      var axed = new AxeService(baseConfig);
      axed.spawn = {};
      axed.spawn.process = new EventEmitter();
      axed.spawn.process.kill = sinon.stub();
      axed.stop(done);
      axed.spawn.process.kill.callCount.should.equal(1);
      axed.spawn.process.kill.args[0][0].should.equal('SIGINT');
      axed.spawn.process.emit('exit', 0);
    });
    it('will give error with non-zero exit status code', function(done) {
      ...
      axed.spawn.process.kill.callCount.should.equal(1);
      axed.spawn.process.kill.args[0][0].should.equal('SIGINT');
      axed.spawn.process.emit('exit', 1);
    });
    it('will stop after timeout', function(done) {
      var axed = new AxeService(baseConfig);
      axed.shutdownTimeout = 300;
      axed.spawn = {};
      axed.spawn.process = new EventEmitter();
      axed.spawn.process.kill = sinon.stub();
      axed.stop(function(err) {
