...
var Proposal = axecore.GovObject.Proposal;
...
/**
 * Provides a friendly event driven API to axed in Node.js. Manages starting and
 * stopping axed as a child process for application support, as well as connecting
 * to multiple axed processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Axe(options) {
  if (!(this instanceof Axe)) {
    return new Axe(options);
  }
...
util.inherits(Axe, Service);

Axe.dependencies = [];

Axe.DEFAULT_MAX_TXIDS = 1000;
Axe.DEFAULT_MAX_HISTORY = 50;
Axe.DEFAULT_MAX_UTXO = 1000;
Axe.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Axe.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Axe.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Axe.DEFAULT_SPAWN_RESTART_TIME = 5000;
Axe.DEFAULT_SPAWN_STOP_TIME = 10000;
Axe.DEFAULT_TRY_ALL_INTERVAL = 1000;
Axe.DEFAULT_REINDEX_INTERVAL = 10000;
Axe.DEFAULT_START_RETRY_INTERVAL = 5000;
Axe.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Axe.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Axe.DEFAULT_INSTANTSEND_FEE = 10000;
Axe.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Axe.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Axe.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Axe.DEFAULT_MAX_HISTORY;
  this.maxUTXOHistory = options.maxUTXOHistory || Axe.DEFAULT_MAX_UTXO;
  this.maxAddressesQuery = options.maxAddressesQuery || Axe.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Axe.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Axe.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Axe.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Axe.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Axe.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Axe.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Axe.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Axe.prototype._initCaches = function() {
...
Axe.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Axe.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Axe.prototype.getPublishEvents = function() {
  return [
    {
...
Axe.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'axed/' + name, 'total:', this.subscriptions[name].length);
};

Axe.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'axed/' + name, 'total:', this.subscriptions[name].length);
};

Axe.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  if(addresses){
    for(var i = 0; i < addresses.length; i++) {
      if (axecore.Address.isValid(addresses[i], this.node.network)) {
        addAddress(addresses[i]);
      }
    }
  }

  log.info(emitter.remoteAddress, 'subscribe:', 'axed/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Axe.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'axed/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Axe.prototype.unsubscribeAddressAll = function(emitter) {
...
Axe.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Axe.DEFAULT_CONFIG_SETTINGS;
...
Axe.prototype._parseAxeConf = function(configPath) {
  var options = {};
...
Axe.prototype._expandRelativeDatadir = function() {
...
Axe.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in axed config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in axed config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in axed config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './axe.conf');

  log.info('Using Axe config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseAxeConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseAxeConf(networkConfigPath));
  }
...
Axe.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of axecore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of axecore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of axecore-node. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in axe.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that axed is undergoing a reindex. ' +
             ...
             'of axecore-node services will start.');
    node._reindex = true;
  }
};

Axe.prototype._resetCaches = function() {
 ...
 * @private
 */
Axe.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Axe.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Axe.prototype._initChain = function(callback) {
  var self = this;
...
Axe.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 9337
  };
  if (this.node.network === axecore.Networks.testnet) {
    networkOptions.rpcport = 19337;
  }
  return networkOptions;
};

Axe.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === axecore.Networks.testnet) {
    networkPath = 'testnet3/axe.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/axe.conf';
    }
  }
  return networkPath;
};

Axe.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === axecore.Networks.testnet) {
...
Axe.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('axed/hashblock', message.toString('hex'));
    }
  }

};

Axe.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Axe.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Axe Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Axe.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Axe.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('axed/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Axe.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = axecore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('axed/rawtransaction', message.toString('hex'));
    }

    var tx = axecore.Transaction();
    tx.fromString(message);
    var txid = axecore.util.buffer.reverse(hash).toString('hex');
    self._notifyAddressTxidSubscribers(txid, tx);

  }
};

Axe.prototype._zmqTransactionLockHandler = function(node, message) {
  var self = this;
  var hash = axecore.crypto.Hash.sha256sha256(message);
  var id = hash.toString('binary');
  if (!self.zmqKnownTransactionLocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.transactionlock.length; i++) {
      this.subscriptions.transactionlock[i].emit('axed/transactionlock', message.toString('hex'));
    }

  }
};

Axe.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Axe.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Axe.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Axe.prototype._initZmqSubSocket = function(node, zmqUrl) {
  var self = this;
...
Axe.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  interval = setInterval(function() {
    node.client.getBlockchainInfo(function(err, response) {
      ...
      log.info('Axe Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
  ...
  }, node._reindexWait || Axe.DEFAULT_REINDEX_INTERVAL);

};

Axe.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Axe.prototype._stopSpawnedAxe = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/axed.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned axe process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean axe process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Axe.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedAxe(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting axe process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Axe process unexpectedly exited with code:', code);
        log.warn('Restarting axe child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Axe process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn axed.'));
      }
...
Axe.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to axed.'));
    }
 ...
 */
Axe.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Axe configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Axe.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Axe.prototype.syncPercentage = function(callback) {
  var self = this;
...
Axe.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, axecore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Axe.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Axe.prototype.getAddressUnspentOutputsPaginated = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = axecore.Script.fromAddress(delta.address);
    return {
 ...
 * @param {Function} callback
 */
Axe.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var addresses = self._normalizeAddressArg(addressArg);
  var cacheKey = addresses.join('');
...
Axe.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Axe.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Axe.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in axecore is most recent to less recent
 ...
 * @param {Function} callback
 */
Axe.prototype.getAddressTxids = function(addressArg, options, callback) {
...
Axe.prototype._getConfirmationsDetail = function(transaction) {
...
Axe.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Axe.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Axe.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A axe transaction id
 * @param {Function} callback
 */
Axe.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Axe.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof axecore.Address) {
...
Axe.prototype._paginate = function(fullArray, fromArg, toArg, fromHeight, toHeight) {
  var slicedArray;
  var filteredArray;
 ...
 * @param {Function} callback
 */
Axe.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
  ...
  self.getAddressTxids(addresses, options, function(err, txids) {
    ...
    try {
      txids = self._paginate(txids, fromArg, toArg);
    } catch(e) {
      return callback(e);
    }
 ...
 * @param {Function} callback
 */
Axe.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
  ...
  function finishWithTxids() {
    if (!options.noTxList) {
      ...
      var paginatedTxids;
      try {
        paginatedTxids = self._paginate(allTxids, fromArg, toArg);
      } catch(e) {
        return callback(e);
      }
 ...
 * @private
 */
Axe.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Axe.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Axe.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a Axecore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
Axe.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = axecore.Block.fromString(response.result);
 ...
 * @param {Function} callback
 */
Axe.prototype.getBlockHashesByTimestamp = function(high, low, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Axe.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 */
Axe.prototype.getBlockHeaders = function(blockArg, callback, nbOfBlockToFetch) {
	var self = this;
	var _toFetch = 25;
 ...
 * @param {Function} callback
 */
Axe.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Axe.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
...
Axe.prototype._getRawTransactionData = function(txid, callback) {
  var self = this;
  self._tryAllClients(function(client, done) {
 ...
 * @param {Function} callback
 */
Axe.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Axecore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Axe.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Axe.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
  ...
  function addTxlockToTx(tx,result) {
    tx.txlock = false;
    if (tx.feeSatoshis >= Axe.DEFAULT_INSTANTSEND_FEE) { // if transaction fee is >= 0.0001 check for txlock
      var rawTx = new axecore.Transaction(result.hex);
      var hash = axecore.crypto.Hash.sha256sha256(rawTx.toBuffer());
      var id = hash.toString('binary');
      if (self.node.services.axed.zmqKnownTransactionLocks.get(id)) {
 ...
 * @param callback
 */
Axe.prototype.govObjectList = function(options, callback) {
  var self = this;
...
Axe.prototype.getCurrentVotes = function (govhash, callback) {
  var self = this;
...
Axe.prototype.getVotes = function (govhash, callback) {
  var self = this;
...
Axe.prototype.getSuperBlockBudget = function (blockindex, callback) {
  var self = this;
...
/*jshint maxparams: 6 */
Axe.prototype.govObjectSubmit = function (parentHash, revision, time, dataHex, feeTxId, callback) {
  var self = this;
...
Axe.prototype.govObjectDeserialize = function (hexdata, callback) {
  var self = this;
...
Axe.prototype.govObjectCheck = function(hexdata, callback){
  var self = this;
...
Axe.prototype.govObjectInfo = function (callback) {
  var self = this;
...
Axe.prototype.govCount = function (callback) {
  var self = this;
...
Axe.prototype.getSpork = function(callback){
	var self = this;
	var SporksList = {};
...
Axe.prototype.getMNList = function(callback){
  var self = this;
  var rawResults= {};
  var MNList = [];
 ...
 * @param callback
 */
Axe.prototype.govObjectHash = function(hash, callback) {
  var self = this;
 ...
 * @param callback
 */
Axe.prototype.govObjectCheck = function govObjectCheck(hexdata, callback) {
  var self = this;
  this.client.gobject('check', hexdata, function (err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
 ...
 * @param {Function} callback
 */
Axe.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Axe.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Axe.prototype.getInfo = function(callback) {
  var self = this;
...
Axe.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Axe.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('axed spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('axed process did not exit'));
      }
