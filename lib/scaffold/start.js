...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for axed is necessary for this upgrade with the "reindex=1" axe.conf option. \n' +
                 'There are changes necessary in both axe.conf and axecore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/axerunners/axecore-node/blob/axed/docs/upgrade.md \n');
        ...
        servicesConfig: {
          axed: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/axed')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed axecore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './axecore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in axecore-node services directory
    ...
    if (servicePackage.axecoreNode) {
      serviceModule = service.name + '/' + servicePackage.axecoreNode;
    }
 ...
 * @param {Node} node - The Axecore Node instance
 */
