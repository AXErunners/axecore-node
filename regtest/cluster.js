...
describe('Axe Cluster', function() {
  var node;
  var daemons = [];
  var execPath = path.resolve(__dirname, process.env.HOME, './.axecore/data/axed')
  var nodesConf = [
    {
      datadir: path.resolve(__dirname, './data/node1'),
      conf: path.resolve(__dirname, './data/node1/axe.conf'),
      rpcuser: 'axe',
    ...
    },
    {
      ...
      conf: path.resolve(__dirname, './data/node2/axe.conf'),
      rpcuser: 'axe',
      rpcpassword: 'local321',
      rpcport: 30522,
    ...
    },
    {
      ...
      conf: path.resolve(__dirname, './data/node3/axe.conf'),
      rpcuser: 'axe',
      rpcpassword: 'local321',
      rpcport: 30523,
  ...
  before(function(done) {
    log.info('Starting 3 axed daemons');
    this.timeout(200000);
  ...
  it('step 1: will connect to three axed daemons', function(done) {
    this.timeout(20000);
    var configuration = {
      network: 'regtest',
      services: [
    ...
    var regtest = axecore.Networks.get('regtest');
  ...
  it('step 2: receive block events', function(done) {
    this.timeout(10000);
    node.services.axed.once('tip', function(height) {
