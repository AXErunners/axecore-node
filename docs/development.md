...
To develop bitcore-node-axe:

```bash
cd ~
git clone git@github.com:<yourusername>/bitcore-node-axe.git
...
We now will setup symlinks in `bitcore-node-axe` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
rm -rf bitcoind-rpc-axe
ln -s ~/bitcoind-rpc-axe
```
...
cd devnode
mkdir node_modules
touch bitcore-node-axe.json
touch package.json
```

Edit `bitcore-node-axe.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../bitcore-node-axe/bin/bitcore-node-axe start
```